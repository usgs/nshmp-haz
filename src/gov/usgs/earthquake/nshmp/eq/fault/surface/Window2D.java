 ...
 * number of rows and columns that you are peering into. <p>
 *
 ...
 * within the ContainerSubset2D.<p>
 *
...
class Window2D {
   ...
   * It can not go past the end col.
   *
   ...
   * original larger matrix. In other words adds the start row to the input row.
   * So if you specify 0 the result would return 20 if 20 was the start row in
   * the original matrix.
   *
   ...
   * original larger matrix.In other words adds the start col to the input col.
   * So if you specify 0 the result would return 20 if 20 was the start col in
   * the original matrix.
   *
   ...
   * shift the window to a smaller starting row. If positive it will shift it to
   * a larger starting row.
   *
   * @param delta The amount you wish to shift the window in index units.
   * @exception ArrayIndexOutOfBoundsException This exception is thrown if the
   *            shift moves the windows either into negative values or outside
   ...
   * shift the window to a smaller col. If positive it will shift it to a larger
   * col.
   *
   * @param delta The amount you wish to shift the window in index units.
   * @exception ArrayIndexOutOfBoundsException This exception is thrown if the
  ...
  /**
   * This function calculates the end row and column based on the user input of
   * start row column and num rows and columns.
   */
   ...
   *
   * @param S Debugging string used for error messaging.
   * @exception ArrayIndexOutOfBoundsException Thrown if any of the window index
   *            information is less than zero.
   */
  ...
  /**
   * Checks that the specified row and column are valid indices into the 2D
   * array. This is a heper function that validates that all start row and start
   * col end row and end col are not larger than the original matrix max number
   * of rows and max number of columns. In other words the window cannot go
   * beyond the bounds of the original matrix.
   *
   * @param S Debugging string used for error messaging.
   * @exception ArrayIndexOutOfBoundsException Thrown if any index of the window
   *            ie. start and end row and col fall beyond the maximum number of
  ...
  /**
   * Helper function used for debugging. This function prints out all variables
   * current values.
   *
