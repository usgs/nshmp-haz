...
public class Regions {
	...
	/**
	 * Creates a {@code Region} from a pair of {@code Location }s. When viewed
	 * in a Mercator projection, the {@code Region} will be a rectangle. If
	 * either both latitude or both longitude values in the {@code Location}s
	 * are the same, an exception is thrown.
	 * 
	 * <p><b>Note:</b> Internally, the size of the region is expanded by a very
	 * small value (~1m) to ensure that calls to
	 * {@link Region#contains(Location)} for any {@code Location} on the north
	 * or east border of the region will return {@code true} and that any double
	 * precision rounding issues do not clip the south and west borders (e.g.
	 * 45.0 may be interpreted as 44.9999...). See also the rules governing
	 * insidedness in the {@link Shape} interface.</p>
	 * 
	 * @param name of the {@code Region}; may be {@code null}
	 ...
	 * If either both latitude or both longitude values in the {@code Location}s
	 * are the same, an exception is thrown. If the supplied
