...
public class IncrementalMfd extends EvenlyDiscretizedFunc {
  ...
  public double getMomentRate(double mag) {
    return getIncrRate(mag) * Earthquakes.magToMoment_N_m(mag);
  }
  ...
  public double getMomentRate(int index) {
    return getIncrRate(index) * Earthquakes.magToMoment_N_m(getX(index));
  }
   ...
   * This returns the object of the class EvenlyDiscretizedFunc which contains
   * all the points with Cum Rate Distribution (the rate greater than and equal
   * to each magnitude). It differs from getCumRateDist() in the X Values
   * because the values are offset by delta/2 in the CumDist returned by this
   * method.
   */
   ...
   * is null then it returns the default Name from the distribution. Makes the
   * call to the parent "getName()" method to get the metadata set outside the
   * application.
   * @return String
   */
  @Override
   ...
   * is null then it returns the default info from the distribution. Makes the
   * call to the parent "getInfo()" method to get the metadata set outside the
   * application.
   * @return String
   */
  @Override
   ...
   * meaning after computing log10 of all y-axis values) between the the given
   * x-axis values. If Double.NaN is passed in, then the first (or last)
   * non-zero rate is used for min_bValMag (or max_bValMag).
   * @param min_bValMag
   * @param max_bValMag
   */
  ...
  /**
   * This sets all y-axis values above the given total moment rate to zero. The
   * final total moment rate will be something less than the value passed in.
   * @param moRate
   */
   ...
   * and largest mags with non-zero rates (zeros at the beginning and end of the
  ...
  /**
   * Returns whether ruptures generated using this MFD should float. May not be
   * applicable to all source types (e.g. grid or point sources).
