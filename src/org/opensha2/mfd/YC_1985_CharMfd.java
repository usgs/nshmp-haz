...
public class YC_1985_CharMfd extends IncrementalMfd {
  ...
  private void calculateRelativeRates() {
	  ...
	  checkArgument(magLower >= minX && magLower <= maxX, "magLower should lie between minX and maxX");
	  checkArgument(magLower <= magUpper, "magLower cannot be less than magUpper");
	  checkArgument(magPrime <= magUpper && magPrime >= magLower, "magPrime must be between magLower and magUpper");
	  checkArgument((magPrime - deltaMagPrime) >= magLower, "magPrime-deltaMagPrime must be greater than magLower");
	  checkArgument(deltaMagChar <= (magUpper-magPrime+deltaMagPrime), "deltaMagChar > (magUpper-magPrime+deltaMagPrime), which is not allowed");
	  checkArgument(magPrime <= (magUpper-deltaMagChar), "magPrime > (magUpper-deltaMagChar), which is not allowed");
