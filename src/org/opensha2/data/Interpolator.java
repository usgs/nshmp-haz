...
public abstract class Interpolator {
  ...
  private static final class RegularInterpolator extends Interpolator {
    ...
    @Override
    public double findY(double[] xs, double[] ys, double x) {
      int i = binaryIndex(xs, x);
      return yFunction.apply(xs[i], ys[i], xs[i + 1], ys[i + 1], x);
    }

    @Override
    public double findY(List<Double> xs, List<Double> ys, double x) {
      int i = binaryIndex(xs, x);
  ...
  private static final class XFn implements InterpolateFn {
    @Override
    public double apply(double x1, double y1, double x2, double y2, double y) {
  ...
  private static final class YFn implements InterpolateFn {
    @Override
    public double apply(double x1, double y1, double x2, double y2, double x) {
