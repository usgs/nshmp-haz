...
public class EvenlyDiscretizedFunc extends AbstractDiscretizedFunc{
	...
	public void add(int index, double y) {
		checkArgument(index >= 0 && index <= (num-1), "The specified index doesn't match this function domain");
		points[index] = y+points[index];
	}
	...
	public double getFirstInterpolatedX(double y){
		...
		checkArgument(found, "Y Value (%s) must be within the range: %s and %s", y, getY(0), getY(num-1));
	...
	public double getInterpolatedY(double x){
		...
		checkArgument(x>=minX-tolerance && x<=maxX+tolerance, "x-value (%s) must be within the range: %s and %s", x, getX(0), getX(num-1));
