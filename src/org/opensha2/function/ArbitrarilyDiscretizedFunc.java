...
public class ArbitrarilyDiscretizedFunc extends AbstractDiscretizedFunc {
   ...
   * stored in a Point2D TreeMap so they are sorted on the X-Values.<p>
   *
   ...
   * most purposes.<p>
   *
   * Note: SWR: I had to make a special org.opensha. version of the Java TreeMap
   * and subclass DataPoint2DTreeMap to access internal objects in the Java
   * TreeMap. Java's Treemap had internal objects hidden as private, I exposed
   * them to subclasses by making them protected in org.opensha.data.TreeMap.
   * This was neccessary for index access to the points in the TreeMap. Seems
   * like a poor oversight on the part of Java.<p>
   */
  ...
  /**
   * Constructor that takes a Point2D Comparator. The comparator is used for
   * sorting the DataPoint2D. Using the no-arg constructor instantiates the
   * default Comparator that compares only x-values within tolerance to
   ...
   * Point2D and usually only compare the x-values for sorting. Special
   ...
   * abstract class in that it calls setTolerance in the tree map which updates
   * the comparator in there.
   *
   * These field getters and setters provide the basic information to describe a
   ...
   * considered equal. A tolerance of less than about 1e-16 is effectively about
   * 1e-16 due to the numerical precision of floating point arithmetic (1.0 -
   * (1.0+1e-16) = 1.0).
   */
  ...
  /**
   * Return the minimum y value along the y-axis. This value is calculated every
   * time a Point2D is added to the list and cached as a variable so this
   * function returns very quickly. Slows down adding new points slightly. I
   * assume that most of the time these lists will be created once, then used
  ...
  /**
   * Return the maximum y value along the y-axis. This value is calculated every
   * time a Point2D is added to the list and cached as a variable so this
   * function returns very quickly. Slows down adding new points slightly. I
   * assume that most of the time these lists will be created once, then used
   * for plotting and in other functions, in other words more lookups than
   ...
   * interpolation is done in the log space. If both bounding y values are zero,
   * then zero is returned. If only one of the bounding y values is zero, that
   * value is converted to Double.MIN_VALUE. If the interpolated y value is
   * Double.MIN_VALUE, it is converted to 0.0.
   ...
   * x1=Double.NaN; double x2=Double.NaN; //if passed parameter(x value) is not
   * within range then throw exception if(x>getX(max-1) || x<getX(0)) throw new
   * InvalidRangeException("x Value must be within the range: " +getX(0)+" and "
   * +getX(max-1)); //if x value is equal to the maximum value of all given X's
   * then return the corresponding Y value if(x==getX(max-1)) return getY(x);
   * //finds the X values within which the the given x value lies for(int
   * i=0;i<max-1;++i) { x1=getX(i); x2=getX(i+1); if(x>=x1 && x<=x2) break; }
   * //finding the y values for the coressponding x values double y1 = getY(x1);
   * double y2 = getY(x2); if(y1==0 && y2==0) return 0; if(y1==0) y1 =
   * Double.MIN_VALUE; if(y2==0) y2 = Double.MIN_VALUE; double
   ...
   * finding the value of y for given x double y= ((logY2-logY1)*(x-x1))/(x2-x1)
   * + logY1; double expY = Math.exp(y); if (expY == Double.MIN_VALUE) expY =
   * 0.0; if(debug) {
   ...
   * @param function DiscretizedFuncAPI function whose Y values will the Y
   *        values of the new ArbitrarilyDiscretizedFunc.
