...
public class Container2DImpl<T> implements Container2D<T> {
	...
	public ListIterator<T> getRowIterator( int column ) throws ArrayIndexOutOfBoundsException {
		if ( column >= numCols ) {
			throw new ArrayIndexOutOfBoundsException("rowIt Column cannot be greater than max index" );
		}
	...
	throws ArrayIndexOutOfBoundsException {
		...
		if ( row >= numRows ) {
			throw new ArrayIndexOutOfBoundsException("Container2D Row cannot be greater than max index: " + numRows );
		}
		if ( column >= numCols ) {
			throw new ArrayIndexOutOfBoundsException("Container2D Column cannot be greater than max index: " + numCols );
		}
	}
