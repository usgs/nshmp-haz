 ...
 * coordinate indices into the real indices of the referenced container.<p>
 *
 ...
 * to minX index in the main container.<p>
 *
 ...
 * oldWindow variable and the commit and rollback functions.<p>
 *
...
class ContainerSubset2D<T> implements Container2D<T> {
   ...
   * container instances. SUbsets can also have names different from the
  ...
  /**
   * The API allows translation, modifications of the Window2D around the larger
   * referenced container. Because there are four variables to the Window2D,
   * this could take four method calls to change the data. Because there is more
   * than one step involved, and any one step can potentially fail, I needed to
   * provide a rollback mechanism to the last know good state. This is the
   * purpose of the oldWindow variable and the commit and rollback functions. .
   */
  ...
  /**
   * Constructor for the ContainerSubset2D object. Provides specifications that
   * allows the Window2D to be fully specified
   *
   ...
   * @exception ArrayIndexOutOfBoundsException Thown if any of the idices go
   *            beyond the range of the referencing container
   */
  ...
  /**
   * Constructor for the ContainerSubset2D object. Provides specifications that
   * allows the Window2D to be fully specified. Also sets the reference to the
   * real data in the main Container2D
   *
   * @param numRows number of index points along the x-axis
   * @param numCols number of index points along the y-axis
   * @param startRow Staring row of this subset, put in Window2D
  ...
  /**
   * Used to update the starting row index in the Window2D. If invalid data, the
   * window is rolled back to the old one.
   *
   * @param startRow The new startRow value
   * @exception ArrayIndexOutOfBoundsException Thrown if this start row is an
   *            invalid index in the main container, i.e. too large or negative.
   */
   ...
   * space.
   *
   * @throws ArrayIndexOutOfBoundsException Thrown if any of the values are
   *         invalid, such as negative indices.
   */
  ...
  /**
   * Used to update the starting cal index in the Window2D. If invalid data, the
   * window is rolled back to the old one.
   *
   * @param startCol The new startCal value in Window2D
   * @exception ArrayIndexOutOfBoundsException Thrown if this start cal is an
   *            invalid index in the main container, i.e. too large or negative.
   */
  ...
  /**
   * Iterate over all cells in the subset window. This iterator starts with the
   * first column and returns all row elements, then moves to the next column,
   * and so on.<p>
   *
  ...
  /**
   * Iterate over all cells in the subset window. This iterator starts with the
   * first row and returns all column elements, then moves to the next row, and
   * so on.<p>
   *
   * Note: This iterates only over the subset indices, not over the full
   * container. The iterator implementation is contained as an inner class in
   ...
   * @exception ArrayIndexOutOfBoundsException Thrown if the shift moves the
   *            window outside the main container boundaries.
   */
   ...
   * succede or the resizing is considered a failure and all operations must be
  ...
  /**
   * Completes a transaction. All required actions completed successfully so the
   * old window variable is deleted and the changes kept in the primary
   * Window2D.
   */
  ...
  //
  // throw new java.lang.UnsupportedOperationException( "This function is not
