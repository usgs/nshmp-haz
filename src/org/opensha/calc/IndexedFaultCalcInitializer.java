...
final class IndexedFaultCalcInitializer implements Callable<GmmInput> {
	...
	@Override public GmmInput call() throws Exception {
		// List<GmmInput> inputs = Lists.newArrayList(); // if aleatory flag
		// or perhaps we have lists of mag or rake variants

		int rjbKey = DataUtils.minKey(rTable.row(DistanceType.R_JB), sectionIDs);
		int rRupKey = DataUtils.minKey(rTable.row(DistanceType.R_RUP), sectionIDs);
		// rRup key is used for rX

		GmmInput.Builder builder = GmmInput.builder();
		// TODO FIX THIS IF THIS TRANSFORM IS ACTUALLY NEEDED
		// .mag(source.mag())
		// .distances(
